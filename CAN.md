...
>[!WARNING]
> The wire colors are not standardized. Always double check the wiring diagram.

Additionally, it is highly recommended for nodes to share a common ground reference.

At the ends of the high and low wires, they are connected via termination resistors (usually 120 ohms).
...
- A voltage spike caused by external noise will affect both wires approximately equally, so the difference between CAN_H and CAN_L will remain unchanged, allowing the receiver to correctly interpret the bits.

Another reason for using two intertwined cables is to reduce the electrical noise emitted by the CAN bus. Since the two wires transmit opposite signals, the electromagnetic waves they produce cancel each other out. To an external observer, it appears as if the CAN bus generates almost no electrical noise.

**Both wires at idle voltage represent a 1, and the wires at different voltages indicate a 0.**
...
- This is why the stub length should be kept under 30 cm—because the nodes do not have termination resistors, signals are subject to reflections, which can return to the main bus.
...
>[!NOTE]
> While knowing the structure of the CAN packet is important for analyzing waveforms by hand, in practice, the packing of the CAN message is usually handled by hardware not software.
